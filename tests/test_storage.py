...
def test_list_by_prefix_top_level(storage: FilesystemStorage):
    """Test listing only top-level notes (empty prefix)."""
...
def test_list_by_prefix_folder(storage: FilesystemStorage):
    ...
    paths = storage.list_by_prefix("projects")
    assert sorted(paths) == ["projects/proj1", "projects/proj2", "projects/sub/proj3"]


def test_list_by_prefix_nested_folder(storage: FilesystemStorage):
    """Test listing notes in a nested folder."""
    ...
    paths = storage.list_by_prefix("projects/sub")
    assert sorted(paths) == ["projects/sub/a", "projects/sub/b"]
...
def test_list_by_prefix_empty_result(storage: FilesystemStorage):
    ...
    paths = storage.list_by_prefix("nonexistent")
    assert paths == []
...
def test_list_by_prefix_strips_slashes(storage: FilesystemStorage):
    ...
    assert storage.list_by_prefix("folder") == ["folder/note"]
    assert storage.list_by_prefix("/folder") == ["folder/note"]
    assert storage.list_by_prefix("folder/") == ["folder/note"]
    assert storage.list_by_prefix("/folder/") == ["folder/note"]
